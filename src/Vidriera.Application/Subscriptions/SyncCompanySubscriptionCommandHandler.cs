...
public class SyncCompanySubscriptionCommandHandler : IRequestHandler<SyncCompanySubscriptionCommand, SyncCompanySubscriptionResult>
{
    ...
    public async Task<SyncCompanySubscriptionResult> Handle(SyncCompanySubscriptionCommand request, CancellationToken cancellationToken)
    {
        ...
        // Se chequea PendingPlan, no PendingPreapprovalId (ver comentario en el webhook handler).
        ...
        MercadoPagoPreapproval? pendingPreapproval = null;
        if (subscription.PendingPreapprovalId is not null)
        {
            pendingPreapproval = await _mercadoPagoClient.GetPreapprovalAsync(subscription.PendingPreapprovalId, cancellationToken);
        ...
        return new SyncCompanySubscriptionResult(
            ...
            preapproval.StartDate,
            preapproval.EndDate,
            pendingPreapproval?.StartDate);
    }
}
