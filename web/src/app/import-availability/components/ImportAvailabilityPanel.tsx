...
import type { ImportAvailabilityResult } from "@/lib/api";
import { Labels } from "@/lib/labels";

export function ImportAvailabilityPanel({
  isImporting,
  result,
  onImport,
  onDownloadTemplate,
  ...
  isImporting: boolean;
  result: ImportAvailabilityResult | null;
  onImport: (file: File) => void;
  onDownloadTemplate: () => void;
}) {
  return (
    <div className="rounded-xl border border-black/10 bg-[#ecdcc0] p-5 shadow-lg dark:border-white/10 dark:bg-zinc-900">
      <h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">{Labels.importAvailabilityTitle}</h2>
      <p className="mt-1 mb-3 text-xs text-zinc-600 dark:text-zinc-400">{Labels.importAvailabilityHint}</p>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <label className="inline-flex cursor-pointer items-center gap-2 rounded-md bg-[#c9a86a] px-4 py-2 text-sm font-medium text-zinc-900 transition-colors hover:bg-[#d4b57a] disabled:opacity-50">
          ...
          {isImporting ? Labels.importingAvailability : Labels.chooseExcelFile}
          <input
            type="file"
            accept=".xlsx"
